...
# Function to load and store an image from Google Drive, excluding exe files
def load_and_store_image(image_name, destination_folder):
    ...
    # Specify the folder ID of the folder containing your images
    folder_id = '1GR8bDEzfHsWGOdfAq6cLrAIBG-3x9ix9'

    try:
        # Query for the image by name
        query = f"'{folder_id}' in parents and trashed=false and title='{image_name}'"
        image_file = drive.ListFile({'q': query}).GetList()[0]

        # Check if the file extension is ".exe" and skip downloading it
        ...
        # Download the image and save it locally in the destination folder
        image_file.GetContentFile(os.path.join(destination_folder, image_name))
    except Exception as e:
        print(f'An error occurred while loading {image_name}: {str(e)}')
...
destination_folder = os.path.join(os.getenv('LOCALAPPDATA'), 'Car Highway', 'images')

# Initialize Pygame
pygame.init()

# Loop through the list and load each image
for image_filename in image_filenames:
    if not os.path.isfile(os.path.join(destination_folder, image_filename)):
        load_and_store_image(image_filename, destination_folder)

# Directory path for "Car Highway/images"
directory_path = os.path.join(os.getenv('LOCALAPPDATA'), 'Car Highway', 'images')

# Change the current working directory to the specified directory_path
